...
class Template:
    ...
    def forward(self):
        ...
        self.model = LLMBackbone(config=self.config)
        ...
        if self.config.zero_shot == True:
            ...
            print(r)
            return
        if self.config.evaluate == True and self.config.lora:
            print("evaluation for saved epoch.")
            r = trainer.final_evaluate_lora(epoch=11)
            trainer.evaluate_step_print()
            print(r)
            return
        elif self.config.evaluate == True:
            print("evaluation for saved epoch.")
            r = trainer.final_evaluate(epoch=11)
            trainer.evaluate_step_print()
            print(r)
            return
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data_name', default='MAMS', choices=['restaurants', 'laptops', 'MAMS'],
                        help='semeval data name')
    parser.add_argument('-f', '--config', default='./config/config.yaml', help='config file')
    parser.add_argument('-e', '--evaluate', default=False, help='whether evaluate using best iter')
    parser.add_argument('-l', '--lora', default=True, help='whether evaluate using best iter')
