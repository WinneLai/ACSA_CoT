...
from src.utils import prompt_for_opinion_inferring, prompt_for_polarity_inferring, prompt_for_polarity_label, prompt_for_opinion_inferring_category, prompt_for_polarity_inferring_category, prompt_for_polarity_label_category
...
class PromptTrainer:
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.config.epoch_size)):
            ...
            if score > best_score:
                ...
                if self.config.lora:
                    peft_model_id= "/home/winnelai/project/ACSA_CoT/data/save/{}_{}_{}/".format(self.config.data_name,  self.config.model_size, epoch)
                    self.model.engine.save_pretrained(peft_model_id)
                else :
                    torch.save({'epoch': epoch, 'model': self.model.state_dict(), 'best_score': best_score},
                           save_name)
            ...
            # self.model.to(self.config.device)
        if self.config.lora:
            res = self.final_evaluate_lora(best_iter)
        else:
    ...
    def final_evaluate_lora(self, epoch=0):
        import pandas as pd
        ...
        model = T5ForConditionalGeneration.from_pretrained(self.config.model_path, load_in_8bit=True, device_map={"":0})
        # Load the Lora model
        self.model.engine = PeftModel.from_pretrained(model, peft_model_id, device_map={"":0})
        self.model.eval()
        res = self.evaluate_step(self.test_loader, mode='test')
        self.add_instance(res)
        return res
...
class ThorTrainer:
    ...
    def prepare_step_two(self, aspect_exprs, data):
        ...
        for context, target, aspect_expr in zip(contexts_A, targets, aspect_exprs):
            if self.config.data_name == "MAMS":
                context_B, prompt = prompt_for_opinion_inferring_category(context, target, aspect_expr)
            else:
    ...
    def prepare_step_three(self, opinion_exprs, data):
        ...
        for context, target, opinion_expr in zip(contexts_B, targets, opinion_exprs):
            if self.config.data_name == "MAMS":
                context_C, prompt = prompt_for_polarity_inferring_category(context, target, opinion_expr)
            else:
    ...
    def prepare_step_label(self, polarity_exprs, pre_cxt, data):
        ...
        # res = {k: v.to(self.config.device) for k, v in res.items()}
        res = {k: v.to("cuda") for k, v in res.items()}
        return res, new_prompts

    def prepare_step_label_catagory(self, opinion_exprs, target_data, data):
        output_ids, output_masks = [data[w] for w in 'output_ids, output_masks'.strip().split(', ')]

        context_B_ids, target_ids = [target_data[w] for w in 'context_B_ids, target_ids'.strip().split(', ')]
        contexts_B = [self.model.tokenizer.decode(ids) for ids in context_B_ids]
        contexts_B = [context.replace('<pad>', '').replace('</s>', '').strip() for context in contexts_B]
        targets = [self.model.tokenizer.decode(ids) for ids in target_ids]
        targets = [context.replace('<pad>', '').replace('</s>', '').strip() for context in targets]
        ...
        new_prompts = []
        for context, target, opinion_expr in zip(contexts_B, targets, opinion_exprs):
            prompt = prompt_for_polarity_label_category(context, target, opinion_expr)
            new_prompts.append(prompt)
    ...
    def train_step(self):
        ...
        for i, data in enumerate(train_data):
            ...
            if self.config.data_name == "MAMS":
                step_label_data,_ = self.prepare_step_label_catagory(step_two_inferred_output, step_one_inferred_data, data)
            else:
                ...
                step_label_data,_ = self.prepare_step_label(step_three_inferred_output, step_two_inferred_data, data)
    ...
    def evaluate_step(self, dataLoader=None, mode='valid'):
        ...
        for i, data in tqdm(enumerate(dataiter), total=dataLoader.data_length):
            with torch.no_grad():
                ...
                step_two_inferred_output = self.model.generate(**step_one_inferred_data)
                if self.config.data_name == "MAMS":
                    # step_label_data, prompt = self.prepare_step_label_catagory(step_two_inferred_output, step_one_inferred_data, data)
                    step_two_inferred_data = self.prepare_step_three(step_two_inferred_output, step_one_inferred_data)
                    step_three_inferred_output = self.model.generate(**step_two_inferred_data)

                    step_label_data, prompt = self.prepare_step_label(step_three_inferred_output, step_two_inferred_data, data)
                else:
                    step_two_inferred_data = self.prepare_step_three(step_two_inferred_output, step_one_inferred_data)
                    step_three_inferred_output = self.model.generate(**step_two_inferred_data)
    ...
    def evaluate_step_print(self, mode='test'):
        path = '/home/winnelai/project/ACSA_CoT/eval/output_{}_{}'.format(self.config.data_name, self.config.model_size)
        with open(path, 'w') as f:
            for i in range(len(self.prompt)):
                f.write(self.prompt[i])
                f.write('\n')
                f.write('pred:{} gold:{}'.format(self.config.label_list[self.preds['total'][i]], self.config.label_list[self.golds['total'][i]]))
                f.write('\n')
        return True
