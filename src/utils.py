...
def prompt_direct_inferring_masked_category(context, target):
    new_context = f'Given the sentence "{context}", '
    prompt = new_context + f'the sentiment polarity of {target} is [mask]'
    return new_context, prompt

def prompt_for_aspect_inferring_category(context, target):
    new_context = f'Given the sentence "{context}", '
    prompt = new_context + f'what part of statement is related to {target}?'
    return new_context, prompt

def prompt_for_opinion_inferring_category(context, target, aspect_expr):
    new_context = context + ' The related statement is ' + aspect_expr + '.'
    prompt = new_context + f' Based on the common knowledge, what is the implicit opinion towards the mentioned aspect category {target}, and why?'
    # prompt = new_context + f' Based on the common knowledge, what is the speaker\'s opinion towards the aspect category {target}, and why?'
    return new_context, prompt

def prompt_for_polarity_inferring_category(context, target, opinion_expr):
    new_context = context + f' The opinion towards the aspect category {target} is ' + opinion_expr + '.'
    prompt = new_context + f' Based on such opinion, what is the sentiment polarity towards {target}?'
    
    return new_context, prompt

def prompt_for_polarity_label_category(context, target, opinion_expr):
    new_context = context + f' The opinion towards the aspect category {target} is ' + opinion_expr + '.'
    prompt = new_context + f' Based on such opinion, what is the sentiment polarity towards {target}? Return the sentiment polarity only, such as positive, neutral, or negative.'
    return prompt
