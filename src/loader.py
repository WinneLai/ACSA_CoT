...
from src.utils import prompt_direct_inferring, prompt_direct_inferring_masked, prompt_for_aspect_inferring, prompt_for_aspect_inferring_category
from src.utils import prompt_direct_inferring_masked_category
...
class MyDataLoader:
    ...
    def data_MAMS(self, path):
        with open(path, "r") as f:
            file = f.readlines()
        data = []
        candidates = {"positive":0, "negative":1, "neutral":2}
        implicit = 0
        for line in file:
            if 'train' in path:
                x, y = line.split("\001")[0], line.strip().split("\001")[1]
                term = y.split(" ")[4]
                golden_polarity = y.split(" ")[6].strip()
            else:
                x, term, golden_polarity = line.split("\001")[0], line.split("\001")[1], line.split("\001")[2].strip()
            data.append([x, term, candidates[golden_polarity], implicit])
        return data
    
    def get_MAMS(self, data=""):
        val_path = "/home/winnelai/project/ACSA_CoT/data/MAMS/MAMS_test.txt"
        train_path = "/home/winnelai/project/ACSA_CoT/data/MAMS/MAMS_train.txt"
        test_path = "/home/winnelai/project/ACSA_CoT/data/MAMS/MAMS_val.txt"
        train_data = self.data_MAMS(train_path)
        test_data = self.data_MAMS(test_path)
        val_data = self.data_MAMS(val_path)
    ...
    def get_data(self):
        ...
        if os.path.exists(path):
            ...
            if cfg.data_name == "MAMS":
                train_data, valid_data, test_data = self.get_MAMS()
                self.data = [train_data, valid_data, test_data]
    ...
    def collate_fn(self, data):
        ...
        if self.config.reasoning == 'prompt':
            ...
            for i, line in enumerate(input_tokens):
                ...
                elif self.config.data_name == 'MAMS':
                    _, prompt = prompt_direct_inferring_masked_category(line, input_targets[i])
                else:
            ...
            for i, line in enumerate(input_tokens):
                ...
                if self.config.data_name == "MAMS":
                    context_step1, prompt = prompt_for_aspect_inferring_category(line, input_targets[i])
                else:
